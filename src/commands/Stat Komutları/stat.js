...
module.exports = {
    ...
    parsher_embed.setThumbnail(member.user.displayAvatarURL({ dynamic: true, size: 2048 }))
    parsher_embed.setDescription(`> **${member.user.toString()} Kullanıcısının <t:${Math.floor(Date.now()/1000)}:R>ki Verileri;**\n⠀\n`)
    parsher_embed.addFields([{name: `${client.emoji("emote_chat") !== null ? client.emoji("emote_chat"):"💭"} • **Toplam Mesaj**`, value: codeBlock("fix",`${messageData ? messageData.topStat : 0} Mesaj`)},
    ...
    parsher_embed.addFields([{name:"⠀", value:`⠀`}]);
    parsher_embed.addFields([{name:`${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • **Sohbet Ettiğin Chat Kanalları:**\n> **(\`Toplam ${chatLength} Kanal\`)**`,value: `${messageTop}`}]);
    parsher_embed.addFields([{name:`${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • **Vakit Geçirdiğin Voice Kanalları:**\n> **(\`Toplam ${voiceLength} Kanal\`)**`,value:`${voiceTop}`}]);
    parsher_embed.setFooter({text:`Menü Üzerinden Verilerinize Filtre Uygulayabilirsiniz`,iconURL:member.user.avatarURL({ dynamic: true, size: 2048 })})

    message.reply({ embeds: [parsher_embed], components: [parsher_dropdown] }).then(b2 => {
            ...
            b2.edit({ content: `> **Menü Zaman Aşımına Uğradı..**`, components: [parsher_dropdown] })
            })
            collector.on('collect', async b => {
                if (!b.isStringSelectMenu()) return;
                const value = b.values[0]
                const daily_pars = new EmbedBuilder()
                ...
                const weekly_pars = new EmbedBuilder()
                ...
                const total_pars = new EmbedBuilder()
                .setDescription(`> **${member.user.toString()} Kullanıcısının Haftalık Ses Ve Mesaj Verileri;**`)
        ...
        const cat_pars = new EmbedBuilder()
        ...
        **${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • Konser & Eğlence: \`${await category(config.parents.funParents)}\`**`}]
        ...
        const camera_pars = new EmbedBuilder()
                ...
                const stream_pars = new EmbedBuilder()
                ...
                const kayıt_pars = new EmbedBuilder()
                ...
                if (value === `anasayfa5`) {
                    await b.update({ embeds: [parsher_embed] })
                }

                if (value === `gun`) {
                    await b.update({ embeds: [daily_pars] })
                }

                if (value === `hafta`) {
                    await b.update({ embeds: [weekly_pars] })
                }

                if (value === `toplam`) {
                    await b.update({ embeds: [total_pars] })
                }

                if (value === `kategori`) {
                    await b.update({ embeds: [cat_pars] })
                }

                if (value === `kamera`) {
                    await b.update({ embeds: [camera_pars] })
                }

                if (value === `yayın`) {
                    await b.update({ embeds: [stream_pars] })
                }

                if (value === `kayıt`) {
                    await b.update({ embeds: [kayıt_pars] })
                }
