...
module.exports = {
    ...
    execute: async (client, message, args, parsher_embed) => {
        ...
        messageUsersData.filter(pars => message.guild.members.cache.get(pars.userId)).splice(0, 10).map(async (x, index) => { messageTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${Number(x.topStat).toLocaleString()}` }) })


        voiceUsersData.filter(pars => message.guild.members.cache.get(pars.userId)).splice(0, 10).map(async (x, index) => voiceTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${moment.duration(x.topStat).format("D [Gün], H [sa], m [dk], s [sn]")}` }))

        messageUsersDataDaily.filter(pars => message.guild.members.cache.get(pars.userId)).splice(0, 10).map(async (x, index) => messageDailyTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${Number(x.dailyStat).toLocaleString()}` }))

        messageUsersDataWeekly.filter(pars => message.guild.members.cache.get(pars.userId)).splice(0, 10).map(async (x, index) => messageWeeklyTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${Number(x.weeklyStat).toLocaleString()}` }))

        voiceUsersDataDaily.filter(pars => message.guild.members.cache.get(pars.userId)).splice(0, 10).map(async (x, index) => voiceDailyTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${moment.duration(x.dailyStat).format("D [Gün], H [sa], m [dk], s [sn]")}` }))

        voiceUsersDataWeekly.filter(pars => message.guild.members.cache.get(pars.userId)).splice(0, 10).map(async (x, index) => voiceWeeklyTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${moment.duration(x.weeklyStat).format("D [Gün], H [sa], m [dk], s [sn]")}` }))

        streamData.filter(pars => message.guild.members.cache.get(pars.userId)).splice(0, 10).map(async (x, index) => streamTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${moment.duration(x.streamStat).format("D [Gün], H [sa], m [dk], s [sn]")}` }))

        cameraData.filter(pars => message.guild.members.cache.get(pars.userId)).splice(0, 10).map(async (x, index) => cameraTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${moment.duration(x.cameraStat).format("D [Gün], H [sa], m [dk], s [sn]")}` }))

        invites.filter(pars => message.guild.members.cache.get(pars.userId)).slice(0, 10).map(async (x, index) => inviteTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${x.Regular}` }))
/*
...
*/
       ...
       // if (registerTop.length > 0 && args[0] && args[0] === "kayıt" || args[0] === "reg" || args[0] === "register" || args[0] === "welcome" || args[0] === "kayıtlar") return mesaj.edit({ content:``,embeds: [returnregister], files: [{ attachment: registerTopCanvas.toBuffer(), name: "return-register.png" }] })
        ...
        mesaj.edit({ content: ``, components: [parsher_dropdown], embeds: [parsher_embed.setDescription(`> **${client.emoji("emote_hi") !== null ? client.emoji("emote_hi") : ""} Aşşağıdaki Menü Üzerinden Sıralama Verilerini Görebilirsiniz!**\n\n> ${client.emoji("emote_warn") !== null ? client.emoji("emote_warn") : "❓"} *Veya \`.top mesaj,ses,davet,yayın,kamera\` vb. Şekilde Belirtebilirsiniz!*`).setThumbnail(message.author.displayAvatarURL({ dynamic: true, size: 2048 }))] })
        ...
        collector.on('end', async b => {
            ...
            mesaj.edit({ content: `> **Menü Zaman Aşımına Uğradı..**`, components: [parsher_dropdown] })
        })
        ...
        collector.on('collect', async b => {
            ...
            if (value == "anasayfa5") {
                b.update({ embeds: [parsher_embed], files: [] })
            }
