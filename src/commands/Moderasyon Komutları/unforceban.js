...
module.exports = {
    ...
    execute: async (client, message, args, parsher_embed) => {
        ...
        if(!forcedata.includes(member.id)) return message.reply({ embeds: [parsher_embed.setDescription(`> **Belirtilen Üyenin Bir Kalıcı Yasaklaması Bulunmamakta!**`)]}).sil(5);
        const fetchBans = message.guild.bans.fetch()
        fetchBans.then(async (bans) => {
        let ban = await bans.find(pars => pars.user.id === member.id)
        if (!ban) {
