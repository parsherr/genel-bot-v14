...
module.exports = {
    ...
    execute: async (client, message, args, parsher_embed) => {
        ...
        if(!tag) return message.reply({ embeds: [parsher_embed.setDescription(`> **${data.map((x,i) => `${i+1}). ${x}`).join("\n")}**`).setTitle("Yasaklı Tag Listesi").setURL(message.url)] })
        if(tag == config.tagSymbol) return message.reply({ embeds: [parsher_embed.setDescription(`> **Sunucunun Tagını Yasaklıya Ekliyemezsin!**`)]}).sil(5);

        if(data.includes(tag)){
        db.pull(`bannedtags`,(eleman, sıra, array) => eleman == tag)
        message.reply({ embeds: [parsher_embed.setDescription(`> **${client.emoji("emote_true") !== null ? client.emoji("emote_true") : "✅"} Belirtilen ${tag} Tagı Yasaklı Taglardan Çıkartıldı!**`)]})
        }else{
        db.push(`bannedtags`,tag)
        message.reply({ embeds: [parsher_embed.setDescription(`> **${client.emoji("emote_true") !== null ? client.emoji("emote_true") : "✅"} Belirtilen ${tag} Tagı Yasaklı Taglara Eklendi!**`)]})
        }
