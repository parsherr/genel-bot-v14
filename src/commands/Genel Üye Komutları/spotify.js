...
module.exports = {
    ...
    execute: async (client, message, args, parsher_embed) => {
        ...
        if (member && member.presence && member.presence.activities && member.presence.activities.some(five => five.name == "Spotify" && five.type == ActivityType.Listening)) {
          ...
          return message.reply({files:[{name:"canvafy.png",attachment:spotify}],embeds: [parsher_embed.setImage('attachment://canvafy.png')] });
        }else{ return message.reply({embeds: [parsher_embed.setDescription(`> **Kullanıcı Spotify Üzerinde Şarkı Dinlemiyor!**`)] });}
