...
module.exports = {
    ...
    execute: async (client, message, args, parsher_embed) => {
        ...
        if (message.member.permissions.has(PermissionFlagsBits.Administrator)) {
            ...
            message.reply({ embeds: [parsher_embed.setDescription(`> **Başarıyla Kullanıcının Kanalına Taşındınız!**`).setColor("#00ff00")] }).sil(5);
            client.true(message)
        } else {
            mesajparsher = message.reply({ content: `> **${member}, ${message.author} \`${member.voice.channel.name}\` Adlı Kanala Gelmek İstiyor, Kabul Ediyomusun?**`, components: [parsher_dropdown] })
