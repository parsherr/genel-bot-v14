...
module.exports = {
    ...
    execute: async (client, message, args, parsher_embed) => {
        if (!config.botOwners.some(pars => message.author.id == pars)) return message.reply({ embeds: [parsher_embed.setDescription(`> **Komutu Kullanmak İçin Yetkin Bulunmamakta!**`)] }).sil(5);
        let secim = args[0];
...
${staffData.length > 0 ? staffData.map((pars) => `<@&${pars}>`).join(",") : "Bulunmamakta"}
**Erkek Rolleri \`ID: 2\`**
${manRoles.length > 0 ? manRoles.map((pars) => `<@&${pars}>`).join(",") : "Bulunmamakta"}
**Kadın Rolleri \`ID: 3\`**
${womanRoles.length > 0 ? womanRoles.map((pars) => `<@&${pars}>`).join(",") : "Bulunmamakta"}
**Kayıtsız Rolleri \`ID: 4\`**
${unregisterRoles.length > 0 ? unregisterRoles.map((pars) => `<@&${pars}>`).join(",") : "Bulunmamakta"}
**Şüpheli Rolleri \`ID: 5\`**
${supheliRoles.length > 0 ? supheliRoles.map((pars) => `<@&${pars}>`).join(",") : "Bulunmamakta"}
**Family/Taglı Rolleri \`ID: 6\`**
${familyRoles.length > 0 ? familyRoles.map((pars) => `<@&${pars}>`).join(",") : "Bulunmamakta"}
**Taglar \`ID: 7\`**
${tagData.length > 0 ? tagData.map((pars) => `${pars}`).join(",") : "Bulunmamakta"}
...
${banstaffData.length > 0 ? banstaffData.map((pars) => `<@&${pars}>`).join(",") : "Bulunmamakta"}
**Jail Yetkilileri \`ID: 11\`**
${jailstaffData.length > 0 ? jailstaffData.map((pars) => `<@&${pars}>`).join(",") : "Bulunmamakta"}
**Voice Mute Yetkilileri \`ID: 12\`**
${vmutestaffData.length > 0 ? vmutestaffData.map((pars) => `<@&${pars}>`).join(",") : "Bulunmamakta"}
**Chat Mute Yetkilileri \`ID: 13\`**
${cmutestaffData.length > 0 ? cmutestaffData.map((pars) => `<@&${pars}>`).join(",") : "Bulunmamakta"}
...
${firstytRoles.length > 0 ? firstytRoles.map((pars) => `<@&${pars}>`).join(",") : "Bulunmamakta"}
**Cezalı / Jailed Rolleri \`ID: 17\`**
${jailRoles.length > 0 ? jailRoles.map((pars) => `<@&${pars}>`).join(",") : "Bulunmamakta"}
...
`).setThumbnail(message.guild.iconURL({ forceStatic: true }))
        ...
        if (!secim) {
            let mesaj = await mesajx.edit({ content:``,components: [secimButtons1, secimButtons2, secimMenu], embeds: [new EmbedBuilder().setImage("attachment://pars-server-setup.png").setColor("Random").setDescription(`> **Bir İşlem Seçiniz!**`)], files: [{ attachment: canvasPanel, name: "pars-server-setup.png" }]})
            ...
            collector.on('collect', async (parsher) => {
                ...
                if (parsher.customId == "regsetup") {
                    parsher.update({ content: ``, embeds: [registerEmbed], components: [parsher_buttons],files:[] })
                    collector.stop();
                } else if (parsher.customId == "modsetup") {
                    parsher.update({ content: ``, embeds: [moderationEmbed], components: [parsher_buttons],files:[] })
                    collector.stop();
                } else if (parsher.customId == "logkur") {
                    parsher.update({ content: `> **📝 Loglar Kuruluyor..**`,embeds:[],files:[], components: [] })
                    const parent = await parsher.guild.channels.create({ name: '</Loglar>', type: ChannelType.GuildCategory });
                    const loglar = config.logs;
                    for (let index = 0; index < loglar.length; index++) {
                        let element = loglar[index];
                        await parsher.guild.channels.create({
                            name: element.name,
                            type: ChannelType.GuildText,
                            parent: parent.id, permissionOverwrites: [
                                { id: parsher.guild.roles.everyone, deny: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages] }
                            ]
                        })
                    }
                    parsher.channel.send({ content: `> **✅ Logların Kurulumu Tamamlandı!**` })
                    collector.stop();
                } else if (parsher.customId == "emojikur") {
                    parsher.update({ content: `> **📝 Emojiler Kuruluyor..**`, embeds:[],files:[],components: [] })
                    const emojiler = config.emojis;
                    for (let index = 0; index < emojiler.length; index++) {
                        let element = emojiler[index];
                        await parsher.guild.emojis.create({
                            name: element.name,
                            attachment: element.url
                        })
                    }
                    parsher.channel.send({ content: `> **✅ Emojilerin Kurulumu Tamamlandı!**` })
                    collector.stop();
                } else if (parsher.customId == "restart") {
                    const five = children.exec(`pm2 restart all`);
                    five.stdout.on('data', async (datas) => {
                        parsher.update({ content: `> 🔃 **Botlar Yeniden Başlatılıyor..**`, components: [],embeds:[],files:[] })
                        collector.stop();
                    });
                } else if (parsher.customId == "datareset") {
                    db.all().forEach(async (data) => {
                        db.delete(data.ID)
                    })
                    parsher.update({ content: `> 🗑️ **Local Veritabanı Siliniyor..**`, components: [] ,embeds:[],files:[]})
                    collector.stop();
                }
            })
            return;
        ...
        if (secim == "1") {
            ...
            if (staffData.some(pars => pars.includes(roles.id))) {
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${config.prefix}setup 1 @rol**` })
            if (manRoles.some(pars => pars.includes(roles.id))) {
            ...
            if (womanRoles.some(pars => pars.includes(roles.id))) {
            ...
            if (unregisterRoles.some(pars => pars.includes(roles.id))) {
            ...
            if (supheliRoles.some(pars => pars.includes(roles.id))) {
            ...
            if (familyRoles.some(pars => pars.includes(roles.id))) {
            ...
            if (tagData.some(pars => pars.includes(tag))) {
            ...
            if (banstaffData.some(pars => pars.includes(roles.id))) {
            ...
            if (jailstaffData.some(pars => pars.includes(roles.id))) {
            ...
            if (vmutestaffData.some(pars => pars.includes(roles.id))) {
            ...
            if (cmutestaffData.some(pars => pars.includes(roles.id))) {
            ...
            if (cmutestaffData.some(pars => pars.includes(roles.id))) {
                ...
                if (jailRoles.some(pars => pars.includes(roles.id))) {
...
let chat = await parsher.channel.send({content:`*Veriler Çekiliyor Lütfen Bekleyiniz, Ve Bu Mesajı Silmeyiniz!*`})
let voice = await parsher.channel.send({content:`*Veriler Çekiliyor Lütfen Bekleyiniz, Ve Bu Mesajı Silmeyiniz!*`})
db.set(`chatleader`,{message:chat.id,channel:parsher.channel.id})
db.set(`voiceleader`,{message:voice.id,channel:parsher.channel.id})
parsher.reply({content:`> **✅ Kurulum Başarılı!** *Leaderboard'un Gözükmesi İçin Biraz Bekleyiniz!*`,ephemeral:true})
 }else if(value == "yasakkaldır"){
    if (!parsher.member.permissions.has(PermissionFlagsBits.Administrator)) return parsher.reply({ content: noPermMessage, ephemeral: true })
    parsher.message.delete();
    const fetchBans = await parsher.guild.bans.fetch()
        fetchBans.forEach(async (bans) => {
        parsher.guild.members.unban(bans.user.id).catch(err => { });
 })
 parsher.channel.send({content:`> **Sunucuda Yasaklı Listesinde Olan Kullanıcıların Banları Açılıyor!**`});
...
if (!parsher.member.permissions.has(PermissionFlagsBits.Administrator)) return parsher.reply({ content: noPermMessage, ephemeral: true })
parsher.message.delete();
parsher.channel.send({components:[rolmenukur1,rolmenukur2,menus],embeds:[new EmbedBuilder().setColor("Random").setFooter({iconURL:"https://cdn.discordapp.com/emojis/1103844452218458212.gif?size=128&quality=lossless",text:`⬆️ = Rol | ⬇️ = Panel`}).setDescription(`> **Merhaba, Hangi Paneli Veya Rolü Kurmak İstiyorsun?**\n\n> **♋ > \`Burç\`**\n> **💝 > \`İlişki\`**\n> **🎉 > \`Etkinlik\`**\n> **🔴 > \`Renk\`**`)]})
}
})
...
client.on(Events.InteractionCreate,async(parsher) => {
    if(!parsher.isStringSelectMenu())return;
     ...
     if(value == "buttonpanel"){
        parsher.message.delete();
        parsher.channel.send({"content": `> ${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} Aşağıdaki Menü Üzerinden Sunucu Hakkında\n> Veya Hesabınızla İlgili Bazı Bilgilere Ulaşabilirsiniz.`,
        "components": [{
            "type": 1, "components": [{
        ...
        parsher.channel.send({content: `> ${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} Aşağıdaki Menü Üzerinden **İstek,Öneri,Şikeyat** Veya **Yetkili Başvurusu** Yapabilirsiniz.`,components:[buttons]})
     }else if(value == "yardımpanel"){
       let cmd = client.commands.find(pars => pars.name == "yardım")
      if(cmd){cmd.execute(client,parsher.message,null,null)}
      parsher.message.delete();
     }
})
client.on(Events.InteractionCreate,async(parsher) => {
 if(!parsher.isButton())return;
  ...
  if(value == "burcrolkur"){
    parsher.reply({ content: `> **📝 Burç Rolleri Kuruluyor..**${parsher.guild.premiumTier >= 2 ? "" : ` *Sunucu Seviyesi 2'den Yüksek Olmadığı İçin Rolleri Iconsuz Kuruyorum!*`}`})
    const burc = config.burcRoles;
    for (let index = 0; index < burc.length; index++) {
        let element = burc[index];
        if(parsher.guild.premiumTier >= 2){
        await parsher.guild.roles.create({
            icon: element.icon,
            name: element.name,
            color: "#CC8899",
          })
        ...
        parsher.channel.send({ content: `> **✅ Burç Rollerinin Kurulumu Tamamlandı!**` })
  }else if(value == "sevrolkur"){
    parsher.reply({ content: `> **📝 İlişki Rolleri Kuruluyor..**${parsher.guild.premiumTier >= 2 ? "" : ` *Sunucu Seviyesi 2'den Yüksek Olmadığı İçin Rolleri Iconsuz Kuruyorum!*`}`})
    ...
    for (let index = 0; index < burc.length; index++) {
        ...
        parsher.channel.send({ content: `> **✅ İlişki Rollerinin Kurulumu Tamamlandı!**` })
  }  else if(value == "renkrolkur"){
    parsher.reply({ content: `> **📝 Renk Rolleri Kuruluyor..**`})
    ...
    for (let index = 0; index < burc.length; index++) {
        ...
        parsher.channel.send({ content: `> **✅ Renk Rollerinin Kurulumu Tamamlandı!**` })
  } else if(value == "etkinlikrolkur"){
    parsher.reply({ content: `> **📝 Etkinlik Rolleri Kuruluyor..**${parsher.guild.premiumTier >= 2 ? "" : ` *Sunucu Seviyesi 2'den Yüksek Olmadığı İçin Rolleri Iconsuz Kuruyorum!*`}`})
    ...
    for (let index = 0; index < burc.length; index++) {
        ...
        parsher.channel.send({ content: `> **✅ Etkinlik Rollerinin Kurulumu Tamamlandı!**` })
  }  else if(value == "burcpanelkur"){
        parsher.message.delete();
        parsher.channel.send({"content": `> ${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} Menü Üzerinden **Burç** Rolünüzü Alabilirsiniz.`,
        "components": [{
            "type": 1, "components": [{
    ...
    parsher.message.delete();
    parsher.channel.send({"content": `> ${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"💖"} Menü Üzerinden **İlişki** Rolünüzü Alabilirsiniz.`,
    "components": [{
        "type": 1, "components": [{
    ...
    parsher.message.delete();
    parsher.channel.send({"content": `> ${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🎉"} Menü Üzerinden **Etkinlik** Rolünüzü Alabilirsiniz.`,
    "components": [{
        "type": 1, "components": [{
    ...
    parsher.message.delete();
    parsher.channel.send({"content": `> ${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} Menü Üzerinden **Renk** Rolünüzü Alabilirsiniz.`,
    "components": [{
        "type": 1, "components": [{
...
client.on(Events.InteractionCreate, async (parsher) => {
    if (!parsher.isButton()) return;
    let fiveValue = parsher.customId;
    ...
    if (fiveValue == "welcome_image") {
        ...
        if (!db.has("five-welcome-image")) {
            ...
            parsher.reply({ content: `> **✅ Canvaslı / Resimli Hoşgeldin Başarıyla Açıldı!**`, ephemeral: true })
        } else {
            db.delete("five-welcome-image");
            parsher.reply({ content: `> **✅ Canvaslı / Resimli Hoşgeldin Başarıyla Kapatıldı!**`, ephemeral: true })
        }
    } else if (fiveValue == "welcome_mentions") {
        if (!parsher.member.permissions.has(PermissionFlagsBits.Administrator)) return parsher.reply({ content: noPermMessage, ephemeral: true })
        if (!db.has("five-welcome-mentions")) {
            db.set("five-welcome-mentions", true);
            parsher.reply({ content: `> **✅ Hoşgeldin Rol Etiketi Başarıyla Açıldı!**`, ephemeral: true })
        } else {
            db.delete("five-welcome-mentions");
            parsher.reply({ content: `> **✅ Hoşgeldin Rol Etiketi Başarıyla Kapatıldı!**`, ephemeral: true })
        }
    } else if (fiveValue == "tag_mode") {
        if (!parsher.member.permissions.has(PermissionFlagsBits.Administrator)) return parsher.reply({ content: noPermMessage, ephemeral: true })
        if (!db.has("five-welcome-tagmode")) {
            db.set("five-welcome-tagmode", true);
            parsher.reply({ content: `> **✅ Taglı Alım Başarıyla Açıldı!**`, ephemeral: true })
        } else {
            db.delete("five-welcome-tagmode");
            parsher.reply({ content: `> **✅ Taglı Alım Başarıyla Kapatıldı!**`, ephemeral: true })
        }
    }
})
