...
module.exports = {
    ...
    execute: async (client, message, args, parsher_embed) => {
        ...
        if(!["ekle","çıkar","liste"].some(x=> args[0] == x))return message.reply({ embeds: [parsher_embed.setDescription(`> **Bir Argüman Belirtin!**\n\n> \`${config.prefix}özel-komut ekle <komut adı> [verilecek RolID] [verebilecek RolID]\`\n> \`${config.prefix}özel-komut çıkar\`\n> \`${config.prefix}özel-komut liste\``)] }).sil(5);
        if(message.mentions.roles.first())return message.reply({ embeds: [parsher_embed.setDescription(`> **Rolleri Etiket İle Değil, ID İle Belirtiniz!**`)] }).sil(5);
        if(args[0] == "ekle"){
        const komutisim = args[1];
        const permRolID = args[2];
        const yetkiRolID = args[3]
        if(!komutisim || !permRolID || !yetkiRolID) return message.reply({ embeds: [parsher_embed.setDescription(`> **Eksiksiz Girdiğinize Emin Olunuz!**\n\n> \`${config.prefix}özel-komut ekle <komut adı> [verilecek RolID] [verebilecek RolID]\``)] }).sil(5);
        const permsData = db.get(`ozelkomutlar`) || [];  //approval,luhux <3
        if(permsData.some(veri=> veri.permName == komutisim)) return message.reply({ embeds: [parsher_embed.setDescription(`> **Bu Komut Zaten Daha Önceden Eklenmiş!**`)] }).sil(5);
        if(permsData.length >= 15) return message.reply({ embeds: [parsher_embed.setDescription(`> **\`15\`'den Fazla Özel Komut Ekliyemezsin!**`)] }).sil(5);
        db.push(`ozelkomutlar`,{permID:permRolID,permName:komutisim,staffRoleID:yetkiRolID})
        message.reply({embeds:[parsher_embed.setDescription(`${codeBlock("md",`# Komut Eklendi!\n< Kullanım: ${config.prefix}${komutisim} <@Luppux/ID>\n> ${komutisim}\n> ${message.guild.roles.cache.get(permRolID).name}\n> ${message.guild.roles.cache.get(yetkiRolID).name} `)}`)]})
        }
        if(args[0] == "çıkar"){
            const permsData = db.get(`ozelkomutlar`) || [];  //approval,luhux <3
        ...
        message.channel.send({components:[menu],embeds:[parsher_embed.setDescription(`> **Listeden Silmek İstediğiniz Komutu Seçin!**`)]}).then(async msg =>{
            ...
            message.reply({ embeds: [parsher_embed.setDescription(`> **\`${liste[index].value}\` Adlı Komut Başarıyla Silindi!**`)]})
            }
            }   
            if(i.values[0] == "iptal") {
        ...
        if(args[0] == "liste"){
            const permsData = db.get(`ozelkomutlar`) || [];  //approval,luhux <3
            message.channel.send({embeds:[parsher_embed.setDescription(`> **${permsData.length == 0 ? "Eklenmiş Özel Komut Bulunmamakta!":`Toplam \`${permsData.length}\` Adet Özel Komut Aşağıda Listelenmiştir!`}**\n\n ${permsData.length == 0 ? " " : `${codeBlock("md",
