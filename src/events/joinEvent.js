...
.build();
member.guild.channels.cache.get(welcomeChannel).send({files:[{attachment: welcome,name: `pars_welcome_${member.id}.png`}],content:parsherWelcomeMessage});
}else{
member.guild.channels.cache.get(welcomeChannel).send({content:parsherWelcomeMessage});
}
} else {
if(jailRoles.length > 0)member.roles.set([...jailRoles]);
